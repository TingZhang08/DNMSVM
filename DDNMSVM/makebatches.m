...
%%%%%导入数据%%%%%%%%
bre_train=load('bre_train10'); % 284*31 

 for i=1:106    % 106
     bre_train(i,1)=-1;
 end
digitdata=bre_train(:,2:end);
targets=bre_train(:,1);

%%%%%% 归一化 %%%%%%%%%
% digitdata=(digitdata-min(min(digitdata)))/(max(max(digitdata))-min(min(digitdata)));

[dataset_scale,ps] = mapminmax(digitdata',0,1);
digitdata = dataset_scale';
...
batchsize = size(digitdata,1);
batchdata = zeros(batchsize, numdims, numbatches);
batchtargets = zeros(batchsize, 1, numbatches);
...
bre_test=load('bre_test10');  % 285*31
for i=1:106    % 106
     bre_test(i,1)=-1;
end
digitdata=bre_test(:,2:end);
targets=bre_test(:,1);
totnum=size(digitdata,1);
...
testbatchtargets = zeros(batchsize, 1, numbatches);
